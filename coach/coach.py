...
class Coach():
    ...
    def __init__(self,joueurs,couleur,side,match):
        self.side = side
    ...
    def changementDePoste(self):
        ...
        distance = []
        closer = None
        for joueur in self.match.joueurs :
        ...
        closer = self.match.joueurs[np.argmin(distance)]
        # print(balle.vitesse())
        ...
        for joueur in self.joueurs:
            ...
            if joueur.poste[-1] == 'WAIT':
                if ball:
                    if baller == joueur:
                        ...
                        if (self.whoIsTheGoal()==joueur)&(joueur.teammate().poste[-1] != 'GOAL'):
                            ...
                            if ((self.side == 'L') & (balle.x < 0)) or ((self.side == 'R') & (balle.x > 0)):
                    ...
                    if (closer.team != self.nom) & (not self.passe or self.lob):
                        ...
                        if self.match.strategie == 'OFFENSIF':
                            if (self.whoIsTheGoal() == joueur) & (joueur.teammate().poste[-1] != 'GOAL'):
                                joueur.defPoste('GOAL')
                                joueur.teammate().defPoste('CHASER')
                            
                            else:
                                joueur.defPoste('CHASER')
                                joueur.teammate().defPoste('GOAL')
                        
                        elif (self.whoIsTheGoal() == joueur) & (joueur.teammate().poste[-1] != 'GOAL'):
                            joueur.defPoste('GOAL')
                ...
                if ball:
                    if (baller in joueur.opponents()) and (balle.vitesse() < 800):
                        joueur.defPoste('WAIT')
                ...
                if ball:
                    if baller==joueur and closer==joueur:
                        joueur.defPoste('ATT')
                        joueur.teammate().defPoste('DEMARQUE')
                ...
                if ball:
                    if baller==joueur:
                        ...
                        joueur.defPoste('DEF2')
                elif (closer==joueur)and(balle.vitesse()<400):
                    joueur.defPoste('CHASER')
                ...
                if ball:
                    if baller==joueur:
                        ...
                        joueur.defPoste('DEF3')
                elif (closer==joueur)and(balle.vitesse()<300):
                    joueur.defPoste('CHASER')
    ...
    def engagement(self):
        if self.match.team_engagement==self.nom:
            if self.side=='R':
                x = 150
            else :
                x = -150
