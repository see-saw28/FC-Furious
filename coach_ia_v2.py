...
class Coach():
    ...
    def whereIstheBall(self):
        ...
        for joueur in self.joueurs[0].match.joueurs :
            ...
            d=joueur.distanceToXY(self.joueurs[0].match.balle.position)
        ...
        for joueur in self.joueurs:
            if joueur.poste[-1]=='SHOOTER':
                ...
                if joueur.status!='EN COURS':
                    ...
                    joueur.status=joueur.commande_position(joueur.goto[0], joueur.goto[1], joueur.teammate().x,joueur.teammate().y)
                ax.plot(joueur.goto[0],joueur.goto[1],'go')
            ...
            #         joueur.goto=(-final_pos[0],final_pos[1])
                ...
                joueur.commande_position(x, 0, 0,0)     
...
class Match():
    ...
    def Vision(self):
        ...
        for robot in self.joueurs:
            ...
            ax.plot(robot.x,robot.y,tag,marker='o',ms=20,label=(str(robot)+' '+str(robot.poste[-1])+' '+str(robot.status)))
        ...
        # ax.add_artist(patches.Rectangle((-longueur, -350), 350, 700, color = 'green'))
